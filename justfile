...
wac_git_url := env_var_or_default("ODEN_WAC_GIT_URL", "https://github.com/mizchi/wac")
wac_git_ref_arg := env_var_or_default("ODEN_WAC_GIT_REF_ARG", "--tag wasmplane-wac-0.10.1-p1")
...
fly_control_url := env_var_or_default("ODENCTL_CONTROL_PLANE_URL", "https://mz-wasmplane-control.fly.dev")
fly_runtime_url := env_var_or_default("ODEN_RUNTIME_URL", "https://mz-wasmplane-runtime.fly.dev")
sample_rust_moonbit_project := env_var_or_default("ODENCTL_SAMPLE_PROJECT_ID", "prj_rust_moonbit_release")
sample_rust_moonbit_host := env_var_or_default("ODENCTL_SAMPLE_HOST", "rust-moonbit.sample.oden.local")
perf_iterations := env_var_or_default("ODENCTL_PERF_ITERATIONS", "20")
perf_warmup := env_var_or_default("ODENCTL_PERF_WARMUP", "2")
perf_concurrency := env_var_or_default("ODENCTL_PERF_CONCURRENCY", "1,4")
perf_nodes := env_var_or_default("ODENCTL_PERF_NODES", "1,2")
perf_history := env_var_or_default("ODENCTL_PERF_HISTORY", "")
...
# Control-plane management and standalone component execution.
odenctl *args:
    pnpm odenctl {{args}}

oden *args:
    cargo run --quiet -p oden --bin oden -- {{args}}
...
installer-test:
    ODEN_INSTALL_E2E=1 node --test tests/installer.test.ts
...
# Binary bodies must survive the deployment adapters before adding image services.
binary-http-build:
    rustup target add wasm32-wasip2
    cargo build --locked --manifest-path examples/binary-http/Cargo.toml --target wasm32-wasip2

binary-http-test: rust-build binary-http-build
    ODEN_BINARY_HOST=target/debug/oden-host ODEN_BINARY_COMPONENT=examples/binary-http/target/wasm32-wasip2/debug/binary_http.wasm node --experimental-strip-types --test tests/worker-binary.test.ts

response-cache-build:
    rustup target add wasm32-wasip2
    cargo build --locked --manifest-path examples/response-cache/Cargo.toml --target wasm32-wasip2

response-cache-test: rust-build response-cache-build
    ODEN_RESPONSE_CACHE_E2E=1 node --experimental-strip-types --test tests/response-cache.test.ts tests/runtime-response-cache.test.ts tests/response-cache-example.test.ts

response-cache-demo: rust-build response-cache-build
    node --experimental-strip-types examples/response-cache/server.ts

# Two immutable site artifacts exercise release and rollback with a real browser.
static-site-build:
    rustup target add wasm32-wasip2
    cargo build --locked --manifest-path examples/static-site/Cargo.toml --target wasm32-wasip2
    cp examples/static-site/target/wasm32-wasip2/debug/static_site_example.wasm examples/static-site/target/site-v1.wasm
    cargo build --locked --manifest-path examples/static-site/Cargo.toml --target wasm32-wasip2 --features release-v2
    cp examples/static-site/target/wasm32-wasip2/debug/static_site_example.wasm examples/static-site/target/site-v2.wasm
...
service-test: rust-build service-rust-build service-moonbit-build
    ODEN_SERVICE_BIN=target/debug/oden ODEN_SERVICE_RUST=examples/service-rust/target/wasm32-wasip2/debug/service_rust.wasm ODEN_SERVICE_MOONBIT=examples/service-moonbit/target/service.wasm node --experimental-strip-types --test tests/service-runtime.test.ts tests/app-manifest.test.ts

# Language-neutral test discovery and runnable WAT, Rust and MoonBit examples.
component-test component *args: rust-build
    target/debug/oden test {{quote(component)}} {{args}}
...
test-runner-test: rust-build test-examples-build
    ...
    target/debug/oden test examples/testing/basic.wat
    target/debug/oden test examples/testing/target/basic.wasm
    ODEN_TEST_BIN=target/debug/oden node --experimental-strip-types --test tests/test-runner-examples.test.ts
...
sdk-test: rust-build service-rust-build service-moonbit-build sdk-pack
    ...
    ODEN_SERVICE_BIN=target/debug/oden ODEN_SERVICE_RUST=examples/service-rust/target/wasm32-wasip2/debug/service_rust.wasm ODEN_SERVICE_MOONBIT=examples/service-moonbit/target/service.wasm node --experimental-strip-types --test tests/sdk-io.test.ts

sdk-celld-test: rust-build service-rust-build service-moonbit-build
    ODEN_CELLD_BIN="${ODEN_CELLD_BIN:-celld}" ODEN_SERVICE_BIN=target/debug/oden ODEN_SERVICE_RUST=examples/service-rust/target/wasm32-wasip2/debug/service_rust.wasm ODEN_SERVICE_MOONBIT=examples/service-moonbit/target/service.wasm node --experimental-strip-types --test tests/sdk-io.test.ts

service-bench-build:
    cargo build --release -p oden --bin oden
    rustup target add wasm32-wasip2
...
app-start manifest: rust-build
    target/debug/oden start {{quote(manifest)}}

app-dev manifest="examples/service-rust/app.json": rust-build
    target/debug/oden dev {{quote(manifest)}}
...
celld-test: rust-build durable-counter-build
    test -n "${ODEN_CELLD_BIN:-}" || (echo "ODEN_CELLD_BIN must point to celld 0.4.1" >&2; exit 1)
    ODEN_STANDALONE_BIN=target/debug/oden ODEN_DURABLE_COMPONENT=examples/durable-counter/target/wasm32-wasip2/debug/durable_counter_example.wasm node --experimental-strip-types --test tests/celld-integration.test.ts
...
run component *args:
    cargo run -p oden --bin oden -- run {{quote(component)}} {{args}}

serve component *args:
    cargo run -p oden --bin oden -- serve {{quote(component)}} {{args}}
...
minimal-moonbit-build: telemetry-bindgen-install
    rm -rf examples/minimal-command/target/moonbit
    "$WIT_BINDGEN" moonbit examples/minimal-command/command.wit --world command --out-dir examples/minimal-command/target/moonbit
    rm -f examples/minimal-command/target/moonbit/gen/interface/wasi/cli/run/stub.mbt
...
minimal-smoke: rust-build minimal-wat-build minimal-moonbit-build
    ODEN_MINIMAL_BIN=target/debug/oden node --experimental-strip-types --test tests/minimal-command.test.ts
...
guest-invoke: rust-build guest-build
    target/debug/oden-host invoke --component "{{ guest_component }}" --method GET --uri http://hello.example.dev/ --body ''

interop-rust-bindings:
    wit-bindgen rust examples/interop/wit --world probe-world --out-dir /tmp/odenctl-rust-interop-wbg
    cp /tmp/odenctl-rust-interop-wbg/probe_world.rs examples/rust-interop/src/bindings.rs
...
sample-rust-moonbit-moonbit-bindings:
    rm -rf "{{ sample_rust_moonbit_dir }}/moonbit-ping/gen" "{{ sample_rust_moonbit_dir }}/moonbit-ping/interface" "{{ sample_rust_moonbit_dir }}/moonbit-ping/world"
    cd "{{ sample_rust_moonbit_dir }}/moonbit-ping" && wit-bindgen moonbit ../wit/ping.wit --world ping-world --out-dir . --derive-show --derive-eq
    cp "{{ sample_rust_moonbit_dir }}/moonbit-ping/src/ping.mbt" "{{ sample_rust_moonbit_dir }}/moonbit-ping/gen/interface/oden/sample/bridge/stub.mbt"
...
sample-rust-moonbit-wac-caller-bindings:
    wit-bindgen rust "{{ sample_rust_moonbit_dir }}/wit/wac-caller.wit" --world wac-caller --out-dir /tmp/odenctl-rust-moonbit-wac-caller-wbg
    cp /tmp/odenctl-rust-moonbit-wac-caller-wbg/wac_caller.rs "{{ sample_rust_moonbit_dir }}/rust-wac-caller/src/bindings.rs"
...
sample-rust-moonbit-smoke: rust-build sample-rust-moonbit-build
    target/debug/oden-host compile --component "{{ sample_rust_moonbit_component }}" --out "{{ sample_rust_moonbit_dir }}/target/rust-moonbit-release.cwasm"
    target/debug/oden-host invoke --component "{{ sample_rust_moonbit_component }}" --method GET --uri "https://{{ sample_rust_moonbit_host }}/sample" --body '' --cpu-ms 1000 --wall-ms 5000 --memory-mb 128 | grep 'moonbit=42'

sample-rust-moonbit-release-preflight:
    test -n "${ODENCTL_CONTROL_PLANE_TOKEN:-}" || (echo "ODENCTL_CONTROL_PLANE_TOKEN is required" >&2; exit 1)
    wac --version
...
sample-rust-moonbit-release: sample-rust-moonbit-release-preflight sample-rust-moonbit-build
    body=$(mktemp); http_status=$(curl -sS -o "$body" -w "%{http_code}" -X POST "{{ fly_control_url }}/projects" -H "authorization: Bearer $ODENCTL_CONTROL_PLANE_TOKEN" -H "content-type: application/json" -d '{"id":"{{ sample_rust_moonbit_project }}","name":"Rust MoonBit release sample"}'); if [[ "$http_status" != "201" && "$http_status" != "409" ]]; then cat "$body"; exit 1; fi
    pnpm odenctl deploy --control-plane-url "{{ fly_control_url }}" --project-id "{{ sample_rust_moonbit_project }}" --component "{{ sample_rust_moonbit_component }}" --host "{{ sample_rust_moonbit_host }}" --path-prefix / --limit cpuMs=1000 --limit wallMs=5000 --limit memoryMb=128 --diff
...
bench: rust-build guest-build
    pnpm bench all --component "{{ guest_component }}" --host-bin target/debug/oden-host --iterations 30 --warmup 3 --concurrency 1,2,4

cluster-bench: rust-build guest-build
    pnpm cluster-bench --component "{{ guest_component }}" --host-bin target/debug/oden-host --nodes 1,2,4 --iterations 30 --warmup 2 --concurrency 1,4,16

cluster-bench-daemon: rust-build guest-build
    pnpm cluster-bench --component "{{ guest_component }}" --host-bin target/debug/oden-host --host-daemon-url http://127.0.0.1:8790 --pooling-total-component-instances 64 --pooling-total-core-instances 256 --pooling-total-memories 64 --pooling-total-tables 128 --pooling-memory-mb 64 --nodes 1,2,4 --iterations 30 --warmup 2 --concurrency 1,4,16

rust-daemon-bench: rust-build guest-build
    pnpm rust-daemon-bench --component "{{ guest_component }}" --host-bin target/debug/oden-host --iterations 300 --warmup 10 --concurrency 1,8,32,64 --http-workers 64 --pooling-total-component-instances 64 --pooling-total-core-instances 256 --pooling-total-memories 64 --pooling-total-tables 128 --pooling-memory-mb 64
...
db-migrate-apply:
    pnpm odenctl migrate apply

pg-migrate: db-migrate-apply

pg-backup output="backups/oden.dump":
    test -n "$DATABASE_URL"
...
host-daemon: rust-build
    target/debug/oden-host serve --host 127.0.0.1 --port 8790 --max-prepared-components 512 --max-concurrent-invocations 64 --pooling-total-component-instances 64 --pooling-total-core-instances 256 --pooling-total-memories 64 --pooling-total-tables 128 --pooling-memory-mb 64
