...
`oden init my-app --language rust` creates an independent application with
a vendored copy of this SDK. `oden build my-app/app.json` builds the component.
The runtime repository is not needed after generation.

Implement `Lifecycle` and `HttpHandler`, then invoke `oden_service_sdk::export!(App)`.
