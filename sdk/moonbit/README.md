...
`oden init my-app --language moonbit` creates an independent project with
this SDK, the WIT contracts and a build script. Run `oden dev my-app/app.json`.
...
handle. The generated `interface/oden/durable/objects` package provides its
...
The package export `@oden/moonbit-service-sdk/build` provides `build(appPath)`.
