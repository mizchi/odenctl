...
Start with `wasmplane check app.json`, `wasmplane --version`, and the server terminal's
logs. This page covers the standalone `wasmplane` binary.

## Commands and builds

| Symptom | Check or fix |
| --- | --- |
| `wasmplane: command not found` | Run `just rust-build` at the repository root and use `target/debug/wasmplane`. Set PATH in each terminal. |
| Missing `dev` or `start` command | `pnpm wasmplane` is the control-plane CLI. Check `target/debug/wasmplane --help`. |
...
For `component does not exist`, run `wasmplane build <app.json>` first. `start` does
...
HTTP component with `run`. For `resident service must export wasmplane:app/lifecycle`,
...
keeps the old app running. Use `wasmplane check app.json` to find unsupported imports
...
`token_env` in **the terminal that starts wasmplane**. Setting the same name in
...
When reporting a bug, include the reproduction command, `wasmplane --version`,
