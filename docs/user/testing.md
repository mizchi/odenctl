...
`oden test` discovers exported functions whose WIT names end in `-test` and
...
```sh
oden test tests.wasm
oden test tests.wat --list
oden test tests.wasm --filter arithmetic-test
oden test tests.wasm --config runtime.json --timeout-ms 2000 --json
```
...
Use `oden test` for component-level assertions and host integration. Keep
...
For changes to oden's test runner or deployment platform, repository regression
and browser tests are described in [developer verification](../developer/verification.md).
