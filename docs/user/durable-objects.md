...
Wasm applications open named objects through the `oden:durable/objects@0.1.0`
WIT interface and call them with asynchronous `fetch`.

The Wasm app runs on oden, while the Durable Object itself runs as JavaScript
...
target/debug/oden run examples/durable-counter/target/wasm32-wasip2/debug/durable_counter_example.wasm \
...
Import the [Durable Object WIT](../../sdk/rust/wit/durable.wit), get a handle with
...
`oden_service_sdk::durable::fetch` and MoonBit's `@service.durable_fetch`.
...
Run `ODEN_CELLD_BIN=/absolute/path/to/celld just celld-test` to verify behavior,
and see the [benchmark guide](../developer/celld-benchmark.md) for performance measurements.
