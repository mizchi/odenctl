...
This guide deploys a standalone `oden start app.json` process on ECS/Fargate.
For artifact upload and route publication through an odenctl control plane,
use [Deploy with odenctl](control-plane.md). The two deployment paths have different
configuration and instance lifetimes.
...
```sh
just aws-image-test
just aws-image-build oden-service:candidate linux/arm64
just aws-container-test oden-service:candidate
