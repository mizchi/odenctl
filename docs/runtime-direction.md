...
execution engine from wasmplane's control plane. The runtime core, standalone CLI, versioned Durable Object
...
The initial CLI targets are `run` for command components and `serve` for HTTP components, implemented as
`wasmplane run` / `wasmplane serve`. Resident lifecycle hooks, single-component manifests, `build/start/dev`,
and Rust/MoonBit SDKs were added in the [service runtime](service-runtime.md).
Managing applications composed of multiple components is future work.
...
| `compile` / `invoke` / `serve` in `crates/wasip3-host/src/main.rs` | Separate standalone CLI and node invocation boundaries |
...
celld owns object ownership, SQLite, persistence, and recovery. wasmplane owns guest execution,
...
The added `wasmplane:durable/objects@0.1.0` contract is:
...
wasmplane projects to fleets; do not assume isolation equivalent to the existing multi-tenant system.
