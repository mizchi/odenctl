...
The Rust and MoonBit SDKs bundled by `wasmplane init` provide standard WASI access
...
Import Rust's `io` and `durable` modules from `wasmplane_service_sdk`.
...
```rust
use wasmplane_service_sdk::{io, types};
...
```rust
use wasmplane_service_sdk::durable;
...
```json
{ "path": "wasmplane/service-sdk/interface/wasmplane/durable/objects", "alias": "objects" }
```
...
`WASMPLANE_CELLD_BIN=/absolute/path/to/celld just sdk-celld-test` starts a disposable
