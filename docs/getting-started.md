...
| Task | Required tools |
| --- | --- |
| Build the runtime and run WAT directly | Git, Rust/Cargo 1.95 or later, a native linker, just |
...
Install Rust and just before starting. The MoonBit examples have been tested with
...
The version output should include `wasmplane` and `Wasmtime 48.0.2`.
The first build takes time to compile Rust dependencies. If you already cloned the
repository, start with `just rust-build` in that directory.

The `export PATH=...` command applies only to the current terminal. In another
terminal, run it again from the repository root or use `target/debug/wasmplane`.
...
```sh
wasmplane init my-app --language rust
rustup target add wasm32-wasip2
wasmplane dev my-app/app.json
```

Use `--language moonbit` for MoonBit. The generated `vendor/wasmplane-sdk` contains
...
```sh
wasmplane build my-app/app.json
wasmplane check my-app/app.json
```
...
wasmplane check examples/service-rust/app.json
