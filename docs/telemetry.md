...
[`wasmplane:telemetry/tracing@0.1.0`](../sdk/rust/wit/telemetry.wit) WIT interface.
...
JSON passed to `wasmplane serve --config`):
...
```rust
use wasmplane_service_sdk::telemetry as t;
...
The output is an ordinary component accepted by `wasmplane serve --resident`.
...
`wasmplane.telemetry.dropped`; export failures increment
`wasmplane.telemetry.export_errors`. Failed batches are discarded, without an
...
This is a versioned wasmplane API, not a claim of stable WASI Observe support.
