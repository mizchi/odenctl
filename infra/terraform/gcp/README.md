# odenctl GCP Terraform POC

This scaffold maps odenctl onto Cloud Run:
...
URL into `ODENCTL_RUNTIME_NODES`. Use GKE when you need the same direct runtime-node publication,
...
terraform -chdir=infra/terraform/gcp plan \
  ...
  -var='control_image=asia-northeast1-docker.pkg.dev/my-gcp-project/odenctl/oden:sha' \
  -var='runtime_image=asia-northeast1-docker.pkg.dev/my-gcp-project/odenctl/oden:sha' \
  -var='database_url_secret_id=odenctl-database-url' \
  -var='api_token_secret_id=odenctl-api-token' \
  -var='runtime_token_secret_id=oden-runtime-token' \
  -var='artifact_access_key_id_secret_id=odenctl-artifact-access-key-id' \
  -var='artifact_secret_access_key_secret_id=odenctl-artifact-secret-access-key'
```
