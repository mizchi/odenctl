...
#[test]
fn runtime_identifies_itself_as_oden() {
    let output = Command::new(env!("CARGO_BIN_EXE_oden"))
        .arg("--version")
        .output()
        .unwrap();
    assert!(output.status.success());
    assert!(String::from_utf8_lossy(&output.stdout).starts_with("oden "));
    let help = Command::new(env!("CARGO_BIN_EXE_oden"))
        .arg("--help")
        .output()
        .unwrap();
    assert!(help.status.success());
    let help = String::from_utf8_lossy(&help.stdout);
    assert!(help.contains("oden run <component>"));
    assert!(help.contains("oden serve <component>"));
    assert!(help.contains("oden test <component>"));
}
...
fn command_runs_without_a_control_plane_and_propagates_failure() {
    ...
    assert!(!dir.path().join("odenctl.sqlite").exists());
}
